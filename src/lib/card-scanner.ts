import type { InsuranceType } from './cost-estimator'
...
export const SCAN_PROMPT = `You are analyzing a health insurance card image.
...
export async function parseScannedCard(raw: string): Promise<ScannedCard> {
  try {
    ...
    throw new Error(
      'Could not parse card data from the image. Try a clearer photo.',
    )
  }
}

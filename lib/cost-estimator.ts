// CPT code lookup for common procedures
export const PROCEDURES: Record<string, { name: string; cpt: string; category: string }> = {
...
// Multipliers applied to the Medicare allowed amount to estimate total billed cost,
// then out-of-pocket based on typical plan structures.
const INSURANCE_CONFIG: Record<InsuranceType, {
  totalCostMultiplier: number;  // Total cost relative to Medicare rate
  coinsurance: number;          // % patient pays after deductible
  typicalDeductible: number;    // Annual deductible (simplified)
  note: string;
}> = {
  "medicare":   { totalCostMultiplier: 1.0, coinsurance: 0.20, typicalDeductible: 240,   note: "20% after Part B deductible ($240/yr in 2024)" },
  "medicaid":   { totalCostMultiplier: 0.7, coinsurance: 0.02, typicalDeductible: 0,     note: "Near-zero cost share; varies by state" },
  "aca-bronze": { totalCostMultiplier: 2.5, coinsurance: 0.40, typicalDeductible: 7000,  note: "40% coinsurance after ~$7,000 deductible" },
  "aca-silver": { totalCostMultiplier: 2.5, coinsurance: 0.30, typicalDeductible: 4500,  note: "30% coinsurance after ~$4,500 deductible" },
  "aca-gold":   { totalCostMultiplier: 2.5, coinsurance: 0.20, typicalDeductible: 1500,  note: "20% coinsurance after ~$1,500 deductible" },
  "employer":   { totalCostMultiplier: 2.2, coinsurance: 0.20, typicalDeductible: 1500,  note: "20% coinsurance after ~$1,500 avg deductible" },
  "uninsured":  { totalCostMultiplier: 3.5, coinsurance: 1.00, typicalDeductible: 0,     note: "Full charge; ask about self-pay discounts (40-60% off)" },
};
...
// Fetch the Medicare allowed amount from CMS Physician Fee Schedule API
async function getMedicareRate(cpt: string): Promise<number | null> {
  try {
    const params = new URLSearchParams({
      "filters[hcpcs_cd]": cpt,
      "filters[place_of_srvc]": "F", // Facility
      size: "1",
      offset: "0",
    });
...
// Fallback rates for when CMS API returns nothing (e.g. G-codes, bundles)
const FALLBACK_RATES: Record<string, number> = {
  "59400": 2300, "59510": 3200, "27447": 1850, "27130": 1800,
  "44950": 650,  "47562": 800,  "66984": 600,  "G0439": 95,
};
...
export async function estimateCost(
  ...
  // Estimate out-of-pocket as a range:
  // Low  = assuming deductible already met for the year
  // High = assuming no deductible has been met yet
  ...
  const outOfPocketHigh = Math.min(
    ...
    Math.round(totalCost) // can't exceed total cost
  );

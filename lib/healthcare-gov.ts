...
const BASE = "https://data.healthcare.gov/api/1/datastore/query";
// ACA QHP landscape dataset resource ID (public, no key required)
...
export async function findPlans(
  ...
  const res = await fetch(`${BASE}/${RESOURCE_ID}`, {
    method: "POST",
  ...
  const results: RawPlan[] = data?.results ?? [];
